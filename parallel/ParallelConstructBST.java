...
class ParallelBuild<T> extends RecursiveTask<Node<T>>{
    ...
    public static <T> Node<T> computeSeq(T[] x, int lo, int hi){
        ...
        current.left = computeSeq(x, lo, mid-1);
    ...
    public Node<T> compute(){
        ...
        int mid = lo+(hi-lo)/2;
        Node<T> current = new Node<>(x[mid], hi-lo+1);
        ParallelBuild<T> left = new ParallelBuild<>(x,lo,mid-1);
        ParallelBuild<T> right = new ParallelBuild<>(x,mid+1,hi);
...
public class ParallelConstructBST{
    ...
    public static void main(String[] args){
        ...
        bst.printTree();
        System.out.println(bst.root.left.right.right.data);
    }
